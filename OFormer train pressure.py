...
def get_arguments():
    ...
    # )
    parser.add_argument(
        '--epochs', type=int, default=140, help='Number of epochs. (default: 100)'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--decoding_depth', type=int, default=1, help='Depth of decoding network in the decoder. (default: 2)'
    )
    parser.add_argument(
        '--fourier_frequency', type=int, default=10, help='Fourier feature frequency. (default: 8)'
    )
    parser.add_argument(
        '--use_grad', action='store_true',
    )
    parser.add_argument(
        '--aug_ratio', type=float, default=0.2, help='Probability to randomly crop'
    )
    # ===================================
    # for dataset
    parser.add_argument(
...
if __name__ == '__main__':
    ...
    train_a = torch.load(f'{DATA_DIR}/dP_train_a.pt')
    train_u = torch.load(f'{DATA_DIR}/dP_train_u.pt')
    val_a = torch.load(f'{DATA_DIR}/dP_val_a.pt')
    val_u = torch.load(f'{DATA_DIR}/dP_val_u.pt')
    print(train_a.shape)
    print(train_u.shape)
    print(val_a.shape)
    print(val_u.shape)
